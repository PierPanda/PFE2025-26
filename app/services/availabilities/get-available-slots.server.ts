...
export async function getAvailableSlots(
  ...
  try {
    ...
    const bookingsResult = await getBookingsByTeacherId(teacherId, [
      "confirmed",
      "pending",
    ]);
    ...
    const slots = availabilitiesResult.availabilities.flatMap(
      (availability) => {
        ...
        const overlappingBookings = bookingsResult.bookings
          ...
          .filter(
            (booking) =>
              booking.endTime > availabilityStart &&
              booking.startTime < availabilityEnd,
          )
        ...
        for (const booking of overlappingBookings) {
          const bookingStart =
            booking.startTime > availabilityStart
              ? booking.startTime
              : availabilityStart;
          const bookingEnd =
            booking.endTime < availabilityEnd
              ? booking.endTime
              : availabilityEnd;
